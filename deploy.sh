...
PROJECT_OPENWEBUI_VERSION="v0.8.12"
PROJECT_OPENWEBUI_IMAGE="ghcr.io/open-webui/open-webui:v0.8.12@sha256:96737272eef841d3dd695d1bbe6e3db5262277609cbc807f9cdd62a6e7c5aa56"
PROJECT_SEARXNG_IMAGE="searxng/searxng:latest@sha256:c9100c29c14a77d5289263a671580226c3b8a396a1a0130d2f500f57076a0119"
...
write_env_file() {
    {
        ...
        env_line CHUTES_PROXY_INTERNAL_URL "$CHUTES_PROXY_INTERNAL_URL"
        ...
        env_line N8N_RUNNERS_MODE "$N8N_RUNNERS_MODE"
        env_line N8N_RUNNERS_AUTH_TOKEN "$N8N_RUNNERS_AUTH_TOKEN"
        ...
        env_line OPENWEBUI_DEFAULT_FEATURE_IDS "$OPENWEBUI_DEFAULT_FEATURE_IDS"
        echo '# Native web search in chat. Compose uses a private SearXNG sidecar with curated no-key engines.'
        env_line ENABLE_WEB_SEARCH "$ENABLE_WEB_SEARCH"
        env_line WEB_SEARCH_ENGINE "$WEB_SEARCH_ENGINE"
        env_line WEB_SEARCH_RESULT_COUNT "$WEB_SEARCH_RESULT_COUNT"
        env_line WEB_SEARCH_CONCURRENT_REQUESTS "$WEB_SEARCH_CONCURRENT_REQUESTS"
        env_line SEARXNG_IMAGE "$SEARXNG_IMAGE"
        env_line SEARXNG_QUERY_URL "$SEARXNG_QUERY_URL"
        env_line SEARXNG_SECRET "$SEARXNG_SECRET"
        env_line WEB_SEARCH_DOMAIN_FILTER_LIST "$WEB_SEARCH_DOMAIN_FILTER_LIST"
        env_line WEB_SEARCH_TRUST_ENV "$WEB_SEARCH_TRUST_ENV"
        env_line WEB_FETCH_MAX_CONTENT_LENGTH "$WEB_FETCH_MAX_CONTENT_LENGTH"
        env_line WEB_LOADER_ENGINE "$WEB_LOADER_ENGINE"
        env_line WEB_LOADER_CONCURRENT_REQUESTS "$WEB_LOADER_CONCURRENT_REQUESTS"
        env_line WEB_LOADER_TIMEOUT "$WEB_LOADER_TIMEOUT"
        env_line ENABLE_WEB_LOADER_SSL_VERIFICATION "$ENABLE_WEB_LOADER_SSL_VERIFICATION"
        env_line BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL "$BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL"
        env_line BYPASS_WEB_SEARCH_WEB_LOADER "$BYPASS_WEB_SEARCH_WEB_LOADER"
        env_line DDGS_BACKEND "$DDGS_BACKEND"
        ...
        env_line IMAGES_OPENAI_API_KEY "$IMAGES_OPENAI_API_KEY"
        env_line IMAGES_OPENAI_API_PARAMS "$IMAGES_OPENAI_API_PARAMS"
        env_line DROPZONE_IMAGE_GENERATION_PROVIDER "$DROPZONE_IMAGE_GENERATION_PROVIDER"
        env_line DROPZONE_IMAGE_FALLBACK_ATTEMPTS "$DROPZONE_IMAGE_FALLBACK_ATTEMPTS"
        env_line DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS "$DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS"
        ...
        env_line DROPZONE_AUDIO_TTS_MAX_CONCURRENCY "$DROPZONE_AUDIO_TTS_MAX_CONCURRENCY"
        env_line DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS "$DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS"
        env_line DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS "$DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS"
        env_line DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS "$DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS"
        env_line DROPZONE_AUDIO_STT_ENGINE "$DROPZONE_AUDIO_STT_ENGINE"
        env_line DROPZONE_AUDIO_STT_LOCAL_MODEL "$DROPZONE_AUDIO_STT_LOCAL_MODEL"
...
for overridable_var in \
    ...
    DROPZONE_IMAGE_GENERATION_PROVIDER \
    DROPZONE_IMAGE_FALLBACK_ATTEMPTS \
    DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS \
    DROPZONE_AUDIO_TTS_MAX_CONCURRENCY \
    DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS \
    DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS \
    DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS \
    DROPZONE_AUDIO_STT_ENGINE \
    DROPZONE_AUDIO_STT_LOCAL_MODEL \
...
ENV_FILE_HAS_SEARXNG_QUERY_URL=false
if [ -f "$ENV_FILE" ] && grep -q '^SEARXNG_QUERY_URL=' "$ENV_FILE"; then
    ENV_FILE_HAS_SEARXNG_QUERY_URL=true
fi
...
SEARXNG_IMAGE="${SEARXNG_IMAGE:-$PROJECT_SEARXNG_IMAGE}"
TZ="${TZ:-UTC}"
...
OPENWEBUI_DEFAULT_FEATURE_IDS="${OPENWEBUI_DEFAULT_FEATURE_IDS:-web_search,image_generation}"
...
if [ "$ENV_FILE_HAS_SEARXNG_QUERY_URL" != "true" ] && \
   [ "${BOOTSTRAP_OVERRIDE_SET_WEB_SEARCH_ENGINE:-false}" != "true" ] && \
   [ "$WEB_SEARCH_ENGINE" = "duckduckgo" ]; then
    WEB_SEARCH_ENGINE="searxng"
fi
WEB_SEARCH_DOMAIN_FILTER_LIST="${WEB_SEARCH_DOMAIN_FILTER_LIST:-[]}"
WEB_SEARCH_TRUST_ENV="${WEB_SEARCH_TRUST_ENV:-false}"
WEB_FETCH_MAX_CONTENT_LENGTH="${WEB_FETCH_MAX_CONTENT_LENGTH:-50000}"
WEB_LOADER_ENGINE="${WEB_LOADER_ENGINE:-safe_web}"
WEB_LOADER_CONCURRENT_REQUESTS="${WEB_LOADER_CONCURRENT_REQUESTS:-4}"
WEB_LOADER_TIMEOUT="${WEB_LOADER_TIMEOUT:-20}"
...
DDGS_BACKEND="${DDGS_BACKEND:-auto}"
...
DROPZONE_IMAGE_FALLBACK_ATTEMPTS="${DROPZONE_IMAGE_FALLBACK_ATTEMPTS:-8}"
DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS="${DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS:-90}"
...
DROPZONE_AUDIO_TTS_MAX_CONCURRENCY="${DROPZONE_AUDIO_TTS_MAX_CONCURRENCY:-1}"
DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS="${DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS:-2}"
DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS="${DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS:-8}"
DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS="${DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS:-2}"
DROPZONE_AUDIO_STT_ENGINE="${DROPZONE_AUDIO_STT_ENGINE:-local}"
DROPZONE_AUDIO_STT_LOCAL_MODEL="${DROPZONE_AUDIO_STT_LOCAL_MODEL:-base}"
...
CHUTES_PROXY_INTERNAL_URL="${CHUTES_PROXY_INTERNAL_URL:-}"
...
if [ "$INSTALL_MODE" = "local" ]; then
    ...
    if [ "$CHUTES_TRAFFIC_MODE" = "e2ee-proxy" ]; then
        ...
        CHUTES_PROXY_INTERNAL_URL="http://e2ee-proxy:80"
...
if [ "$CHUTES_TRAFFIC_MODE" = "e2ee-proxy" ]; then
    IMAGES_OPENAI_API_BASE_URL="${CHUTES_PROXY_INTERNAL_URL%/}/v1"
elif [ "${BOOTSTRAP_OVERRIDE_SET_IMAGES_OPENAI_API_BASE_URL:-false}" != "true" ] && \
    { [ -z "${IMAGES_OPENAI_API_BASE_URL:-}" ] || is_proxy_backed_openwebui_url "$IMAGES_OPENAI_API_BASE_URL"; }; then
    IMAGES_OPENAI_API_BASE_URL="$OPENWEBUI_API_BASE_URL"
fi
...
if [ -z "${N8N_RUNNERS_AUTH_TOKEN:-}" ] || [ "$FORCE_ALL" = true ]; then
    N8N_RUNNERS_AUTH_TOKEN="$(generate_hex 32)"
fi
if [ -z "${WEBUI_SECRET_KEY:-}" ] || [ "$FORCE_ALL" = true ]; then
    WEBUI_SECRET_KEY="$(generate_hex 32)"
fi
if [ -z "${SEARXNG_SECRET:-}" ] || [ "$FORCE_ALL" = true ]; then
    SEARXNG_SECRET="$(generate_hex 32)"
fi
if [ -z "${DROPZONE_AUDIO_INTERNAL_API_KEY:-}" ] || [ "$FORCE_ALL" = true ]; then
    DROPZONE_AUDIO_INTERNAL_API_KEY="$(generate_hex 32)"
fi
