...
def patch_utils_oauth(path: Path) -> None:
    ...
    patched = replace_one_of_or_keep(
        ...
        "\n"
        "                    if provider_status_code is not None and provider_status_code >= 500 and token_attempt < 4:\n"
        "                        if callback_state_key and callback_state_data is not None:\n"
        "                            request.session[callback_state_key] = callback_state_data\n"
        "                        log.warning(\n"
        "                            'OAuth token exchange returned provider status %s; retrying attempt %s/5',\n"
        "                            provider_status_code,\n"
        "                            token_attempt + 2,\n"
        "                        )\n"
        "                        await asyncio.sleep(0.75 * (token_attempt + 1))\n"
        "                        continue\n"
        "\n"
        "                    detailed_error = _build_oauth_callback_error_message(e)\n"
        "                    log.warning(\n"
    ...
    patched = replace_one_of_or_keep(
        ...
        "                user_data = None\n"
        "                userinfo_provider_status_code = None\n"
        "                for userinfo_attempt in range(5):\n"
        "                    try:\n"
        "                        user_data: UserInfo = await client.userinfo(token=token)\n"
        "                        break\n"
        "                    except Exception as e:\n"
        ...
        "                        except (TypeError, ValueError):\n"
        "                            provider_status_code = None\n"
        "                        userinfo_provider_status_code = provider_status_code\n"
        "\n"
        "                        if provider_status_code is not None and provider_status_code >= 500 and userinfo_attempt < 4:\n"
        "                            log.warning(\n"
        "                                'OAuth userinfo returned provider status %s; retrying attempt %s/5',\n"
        "                                provider_status_code,\n"
        "                                userinfo_attempt + 2,\n"
        "                            )\n"
        "                            await asyncio.sleep(0.75 * (userinfo_attempt + 1))\n"
        "                            continue\n"
        "\n"
        "                        if provider_status_code is not None and provider_status_code >= 500:\n"
        "                            break\n"
    ...
    patched = insert_after_one_of(
        ...
        "                if not user_data and provider == 'oidc':\n"
        "                    access_token = str(token.get('access_token') or '').strip() if isinstance(token, dict) else ''\n"
        "                    issuer = str(getattr(client, 'server_metadata', {}).get('issuer') or '').strip().rstrip('/')\n"
        "                    if access_token and issuer.endswith('/idp'):\n"
        "                        users_me_url = f\"{issuer[:-4]}/users/me\"\n"
        "                        try:\n"
        ...
        "                                        sub = str(account_data.get('user_id') or account_data.get('sub') or '').strip()\n"
        "                                        username = str(account_data.get('username') or account_data.get('name') or sub).strip()\n"
        "                                        if sub:\n"
        "                                            user_data = UserInfo(\n"
        "                                                {\n"
        "                                                    'sub': sub,\n"
        ...
        "                                                }\n"
        "                                            )\n"
        "                                            log.warning('OAuth userinfo unavailable; used Chutes account profile fallback')\n"
        "                                    else:\n"
        "                                        log.warning('OAuth userinfo fallback returned status %s', resp.status)\n"
        "                        except Exception:\n"
        "                            log.warning('OAuth userinfo fallback failed', exc_info=True)\n"
        "                if (\n"
        "                    not user_data\n"
        "                    and userinfo_provider_status_code is not None\n"
        "                    and userinfo_provider_status_code >= 500\n"
        "                ):\n"
        "                    raise HTTPException(\n"
        "                        502,\n"
        "                        detail='OAuth provider is temporarily unavailable. Please try again in a minute.',\n"
        "                    )\n",
        "oauth callback Chutes userinfo fallback",
    )
    patched = replace_one_of_or_keep(
        patched,
        [
