...
write_env_file() {
    {
        ...
        env_line N8N_RUNNERS_MODE "${N8N_RUNNERS_MODE:-internal}"
        ...
        env_line WEB_LOADER_ENGINE "${WEB_LOADER_ENGINE:-safe_web}"
        env_line WEB_LOADER_CONCURRENT_REQUESTS "${WEB_LOADER_CONCURRENT_REQUESTS:-4}"
        env_line WEB_LOADER_TIMEOUT "${WEB_LOADER_TIMEOUT:-20}"
        env_line ENABLE_WEB_LOADER_SSL_VERIFICATION "${ENABLE_WEB_LOADER_SSL_VERIFICATION:-true}"
        env_line BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL "${BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL:-false}"
        env_line BYPASS_WEB_SEARCH_WEB_LOADER "${BYPASS_WEB_SEARCH_WEB_LOADER:-false}"
        env_line DDGS_BACKEND "${DDGS_BACKEND:-auto}"
        ...
        env_line DROPZONE_IMAGE_FALLBACK_ATTEMPTS "${DROPZONE_IMAGE_FALLBACK_ATTEMPTS:-8}"
        env_line DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS "${DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS:-90}"
        ...
        env_line DROPZONE_AUDIO_TTS_MAX_CONCURRENCY "${DROPZONE_AUDIO_TTS_MAX_CONCURRENCY:-1}"
        env_line DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS "${DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS:-2}"
        env_line DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS "${DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS:-5}"
        env_line DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS "${DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS:-1}"
...
if [ -f "$SENTINEL" ] && [ "$RECONFIGURE" != true ]; then
    ...
    if [ "$CHUTES_TRAFFIC_MODE" = "e2ee-proxy" ]; then
        ...
        CHUTES_PROXY_INTERNAL_URL="http://127.0.0.1:8081"
    ...
    elif [ -z "${IMAGES_OPENAI_API_BASE_URL:-}" ] || is_proxy_backed_openwebui_url "$IMAGES_OPENAI_API_BASE_URL"; then
    ...
    N8N_RUNNERS_AUTH_TOKEN="${N8N_RUNNERS_AUTH_TOKEN:-$(generate_hex 32)}"
    ...
    DROPZONE_AUDIO_INTERNAL_API_KEY="${DROPZONE_AUDIO_INTERNAL_API_KEY:-$(generate_hex 32)}"
...
if [ "$CHUTES_TRAFFIC_MODE" = "e2ee-proxy" ]; then
    IMAGES_OPENAI_API_BASE_URL="${CHUTES_PROXY_INTERNAL_URL:-http://127.0.0.1:8081}"
    IMAGES_OPENAI_API_BASE_URL="${IMAGES_OPENAI_API_BASE_URL%/}/v1"
else
...
export N8N_RUNNERS_MODE="${N8N_RUNNERS_MODE:-internal}"
export N8N_RUNNERS_AUTH_TOKEN="${N8N_RUNNERS_AUTH_TOKEN:-}"
if [ "$N8N_RUNNERS_MODE" = "external" ]; then
    export N8N_RUNNERS_BROKER_LISTEN_ADDRESS=0.0.0.0
fi
export N8N_COMMUNITY_PACKAGES_ENABLED=false
export N8N_VERIFIED_PACKAGES_ENABLED=false
export N8N_UNVERIFIED_PACKAGES_ENABLED=false
export NODES_EXCLUDE='["n8n-nodes-base.executeCommand","n8n-nodes-base.localFileTrigger","n8n-nodes-base.readWriteFile"]'
...
# Internal service-to-service URL for the e2ee-proxy sidecar (openresty on
# 127.0.0.1:8081 inside this container). Distinct from CHUTES_PROXY_BASE_URL
# which is the externally-facing URL used by token-test flows.
if [ "$CHUTES_TRAFFIC_MODE" = "e2ee-proxy" ]; then
    export CHUTES_PROXY_INTERNAL_URL="${CHUTES_PROXY_INTERNAL_URL:-http://127.0.0.1:8081}"
else
    export CHUTES_PROXY_INTERNAL_URL=""
fi
...
export WEB_LOADER_ENGINE="${WEB_LOADER_ENGINE:-safe_web}"
export WEB_LOADER_CONCURRENT_REQUESTS="${WEB_LOADER_CONCURRENT_REQUESTS:-4}"
export WEB_LOADER_TIMEOUT="${WEB_LOADER_TIMEOUT:-20}"
export ENABLE_WEB_LOADER_SSL_VERIFICATION="${ENABLE_WEB_LOADER_SSL_VERIFICATION:-true}"
export BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL="${BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL:-false}"
export BYPASS_WEB_SEARCH_WEB_LOADER="${BYPASS_WEB_SEARCH_WEB_LOADER:-false}"
export DDGS_BACKEND="${DDGS_BACKEND:-auto}"
...
if [ "$CHUTES_TRAFFIC_MODE" = "e2ee-proxy" ]; then
    export IMAGES_OPENAI_API_BASE_URL="${CHUTES_PROXY_INTERNAL_URL%/}/v1"
else
...
export DROPZONE_IMAGE_FALLBACK_ATTEMPTS="${DROPZONE_IMAGE_FALLBACK_ATTEMPTS:-8}"
export DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS="${DROPZONE_IMAGE_FAILURE_COOLDOWN_SECONDS:-90}"
...
export DROPZONE_AUDIO_TTS_MAX_CONCURRENCY="${DROPZONE_AUDIO_TTS_MAX_CONCURRENCY:-1}"
export DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS="${DROPZONE_AUDIO_TTS_MIN_INTERVAL_SECONDS:-2}"
export DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS="${DROPZONE_AUDIO_TTS_RETRY_ATTEMPTS:-5}"
export DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS="${DROPZONE_AUDIO_TTS_RETRY_BASE_DELAY_SECONDS:-1}"
export DROPZONE_AUDIO_STT_ENGINE="${DROPZONE_AUDIO_STT_ENGINE:-local}"
export DROPZONE_AUDIO_STT_LOCAL_MODEL="${DROPZONE_AUDIO_STT_LOCAL_MODEL:-base}"
